-- [[ COFFEE VIP HUB - ADDED ANTI-STUN ]] --
...
-- ScrollingFrame (ໜ້າຕ່າງເລື່ອນລົງໄດ້ - ເພີ່ມ CanvasSize ເພື່ອຮອງຮັບປຸ່ມໃໝ່)
...
-- 4. Hitbox Extender Button (Big Red Box)
...
DmgBtn.TextSize = 11
DmgBtn.Font = Enum.Font.SourceSansBold
...
FastAtkBtn.TextSize = 11
FastAtkBtn.Font = Enum.Font.SourceSansBold
Instance.new("UICorner", FastAtkBtn).CornerRadius = UDim.new(0, 6)

-- 8. NEW: Anti-Stun Button (ບໍ່ຕິດສະຕັ້ນ)
local AntiStunBtn = Instance.new("TextButton", ContentContainer)
AntiStunBtn.Size = UDim2.new(0, 210, 0, 30)
AntiStunBtn.Position = UDim2.new(0, 15, 0, 324)
AntiStunBtn.Text = "🛡️ Anti-Stun: OFF"
AntiStunBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
AntiStunBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
AntiStunBtn.TextSize = 11
AntiStunBtn.Font = Enum.Font.SourceSansBold
Instance.new("UICorner", AntiStunBtn).CornerRadius = UDim.new(0, 6)
...
-- Hitbox Extender Logic (Big Red Box)
...
AntiStunBtn.MouseButton1Click:Connect(function()
    ...
    if antiStunActive then
        AntiStunBtn.Text = "🛡️ Anti-Stun: ON"
        AntiStunBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 80)
    else
        AntiStunBtn.Text = "🛡️ Anti-Stun: OFF"
        AntiStunBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
    end
...
task.spawn(function()
    while task.wait(0.2) do
        if antiStunActive and LP.Character then
            pcall(function()
                -- ລົບວັດຖຸ/ຄ່າສະຕັ້ນທີ່ມັກຖືກແຊກເຂົ້າມາໃນຕົວລະຄອນ
                for _, obj in ipairs(LP.Character:GetChildren()) do
                    if obj.Name:lower():find("stun") or obj.Name:lower():find("knockback") or obj.Name:lower():find("flinch") then
                        obj:Destroy()
                    end
                end
                -- ປ້ອງກັນ PlatformStanding ຫຼື State ທີ່ເຮັດໃຫ້ລົ້ມ/ສະຕັ້ນ
