...
Title.Text = "🔐 ໃສ່ລະຫັດເຂົ້າໃຊ້ງານ"
...
TextBox.PlaceholderText = "ພິມລະຫັດທີ່ນີ້..."
TextBox.Text = ""
...
mkMenuBtn("⚡ ແລ່ນໄວ (Speed 1000)", 15, function()
    if LP.Character and LP.Character:FindFirstChild("Humanoid") then
        LP.Character.Humanoid.WalkSpeed = 1000
    end
end)

mkMenuBtn("🔄 ຕັ້ງຄ່າປົກກະຕິ", 55, function()
    if LP.Character and LP.Character:FindFirstChild("Humanoid") then
        LP.Character.Humanoid.WalkSpeed = 16
    end
end)

mkMenuBtn("❌ ปິດเมນູ", 95, function()
    SG:Destroy()
end)
...
SubmitBtn.MouseButton1Click:Connect(function()
    if TextBox.Text == "1234" then -- ລະຫັດຜ່ານແມ່ນ 1234 (ສາມາດປ່ຽນໄດ້)
        LoginFrame.Visible = false
        MainFrame.Visible = true
    else
        TextBox.Text = ""
        TextBox.PlaceholderText = "ລະຫັດຜ່ານບໍ່ຖືກ!"
    end
end)
